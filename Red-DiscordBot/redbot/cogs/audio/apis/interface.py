...
class AudioAPIInterface:
    ...
    async def get_random_track_from_db(self, tries=0) -> Optional[MutableMapping]:
        ...
        try:
            ...
            debug_exc_log(
                log, exc, "Failed to fetch a random track from database")
            track = {}
    ...
    async def run_tasks(self, ctx: Optional[commands.Context] = None, message_id=None) -> None:
        ...
        async with self._lock:
            if lock_id in self._tasks:
                if IS_DEBUG:
                    log.debug("Running database writes for %d (%s)",
                              lock_id, lock_author)
                try:
                    ...
                    if IS_DEBUG:
                        log.debug(
                            "Completed database writes for %d (%s)", lock_id, lock_author)
    ...
    async def fetch_from_spotify_api(
        ...
        while True:
            ...
            if query_type == "track":
                ...
                if tracks_raw:
                    new_tracks = [k["track"]
                                  for k in tracks_raw if k.get("track")]
                    tracks.extend(new_tracks)
    ...
    async def fetch_track(
        ...
        if cache_enabled and not forced and not query.is_local:
            try:
                ...
                debug_exc_log(
                    log, exc, "Failed to fetch %r from Lavalink table", query_string)
    ...
    async def autoplay(self, player: lavalink.Player, playlist_api: PlaylistWrapper):
        ...
        if autoplaylist["enabled"]:
            try:
                ...
                debug_exc_log(
                    log, exc, "Failed to fetch playlist for autoplay")
        ...
        if not tracks or not getattr(playlist, "tracks", None):
            ...
            if not tracks:
                ...
                (results, called_api) = await self.fetch_track(
                    cast(commands.Context, ctx(
                        player.guild, player.guild, self.cog)),
                    player,
                    Query.process_input(
        ...
        if tracks:
            ...
            while valid is False and multiple:
                ...
                query = Query.process_input(
                    track, self.cog.local_folder_current_path)
