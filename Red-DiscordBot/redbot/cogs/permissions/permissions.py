                    ...
                    Or(str, int): Or(
                        {
                            Or(int, "default"): And(
                                bool, error=_(
                                    "Rules must be either `true` or `false`.")
                            )
                        },
                        {},
                        error=_(
...
class Permissions(commands.Cog):
                    ...
                    if _uid in guild_rules:
                        if obj:
                            ...
                            await self._remove_rule(
                                CogOrCommand(
                                    typename, obj.qualified_name, obj),
                                user_id,
                                int(guild_id),
                            )
                        else:
    ...
    async def _permissions_acl_set(
        ...
        try:
            ...
            await ctx.send(
                _("Your YAML file did not match the schema: ") +
                translate(e.errors[-1])
            )
        else:
    ...
    async def _yaml_set_acl(self, source: discord.Attachment, guild_id: int, update: bool) -> None:
        ...
        for category, getter in ((COG, self.bot.get_cog), (COMMAND, self.bot.get_command)):
            ...
            for cmd_name, cmd_rules in rules_dict.items():
                cmd_rules = {str(model_id): rule for model_id,
                             rule in cmd_rules.items()}
        ...
        for guild_id, guild_dict in _int_key_map(rule_dict.items()):
            for model_id in guild_dict.keys():
                if model_id == "default":
                    ...
                    cog_or_command.clear_rule_for(
                        int(model_id), guild_id=guild_id)
