...
class Core(commands.commands._RuleDropper, commands.Cog, CoreLogic):
    ...
    async def info(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            title=f"RedBot v{__version__}",
            description=(f"**Instance Owner** {'*Owned by Team*' if app_info.team else str(owner)}"
                         f"\n\n**discord.py** {dpy_version}"
                         f"\n**Python** {python_version}"),
            color=(await ctx.embed_colour())
        ).set_thumbnail(url=ctx.me.display_avatar)
    ...
    async def mydata_3rd_party(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        if no_statements:
            parts.append(
                "## " + _("3rd-party extensions without statements\n"))
    ...
    async def mydata_forgetme(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        if ctx.assume_yes:
            ...
            # We will however not let that lock them out either.
            ctx.command.reset_cooldown(ctx)
            return await ctx.send(
        ...
        if results.unhandled:
            await ctx.send(
                _("{mention} The following cogs did not handle deletion:\n{cogs}.").format(
                    mention=ctx.author.mention, cogs=humanize_list(
                        results.unhandled)
                )
            )
    ...
    async def embedset_showsettings(
        ...
        **Arguments:**
        ...
        if ctx.guild:
            ...
            if command_name is not None:
                scope = self.bot._config.custom(
                    "COMMAND", command_name, ctx.guild.id)
    ...
    async def load(self, ctx: commands.Context, *cogs: str):
        ...
        **Arguments:**
        ...
        if loaded:
            loaded_packages = humanize_list(
                [inline(package) for package in loaded])
        ...
        if already_loaded:
            if len(already_loaded) == 1:
                ...
                formed = _("The following packages are already loaded: {packs}").format(
                    packs=humanize_list([inline(package)
                                        for package in already_loaded])
                )
            output.append(formed)
    ...
    async def set_showsettings(self, ctx: commands.Context):
        ...
        if ctx.guild:
            ...
            admin_role_names = [
                r.name for r in guild.roles if r.id in admin_role_ids]
            admin_roles_str = humanize_list(
                admin_role_names) if admin_role_names else "Not Set."
            mod_role_ids = guild_data["mod_role"]
            mod_role_names = [
                r.name for r in guild.roles if r.id in mod_role_ids]
            mod_roles_str = humanize_list(
                mod_role_names) if mod_role_names else "Not Set."
    ...
    async def _game(self, ctx: commands.Context, *, game: str = None):
        ...
        **Arguments:**
        ...
        if game:
            ...
            game = None
        status = ctx.bot.guilds[0].me.status if len(
            ctx.bot.guilds) > 0 else discord.Status.online
    ...
    async def status(self, ctx: commands.Context, *, status: str):
        ...
        **Arguments:**
        ...
        game = ctx.bot.guilds[0].me.activity if len(
            ctx.bot.guilds) > 0 else None
        try:
    ...
    async def api_list(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        joined = _("Set API services:\n") if len(
            services) > 1 else _("Set API service:\n")
        ...
        for destination in destinations:
            ...
            if send_embed:
                ...
                try:
                    ...
                    log.exception(
                        f"Contact failed to {destination}({destination.id})")
    ...
    async def dm(self, ctx: commands.Context, user_id: int, *, message: str):
        ...
        **Arguments:**
        ...
        content = _(
            "You can reply to this message with {}contact").format(prefix)
    ...
    async def debuginfo(self, ctx: commands.Context):
        ...
        if IS_WINDOWS:
            ...
            osver = f"{distro.name()} {distro.version()}".strip()  # type: ignore
        else:
    ...
    async def diagnoseissues(
        ...
        **Arguments:**
        ...
        if not channel.permissions_for(member).send_messages:
            ...
            return
        issue_diagnoser = IssueDiagnoser(
            self.bot, ctx, channel, member, command)
    ...
    async def localallowlist_add(
        ...
        **Arguments:**
        ...
        if not (ctx.guild.owner == ctx.author or await self.bot.is_owner(ctx.author)):
            ...
            ids = {i for i in (
                ctx.author.id, *(getattr(ctx.author, "_roles", [])))}
    ...
    async def localallowlist_list(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        for obj_id in curr_list:
            user_or_role = self.bot.get_user(
                obj_id) or ctx.guild.get_role(obj_id)
            if not user_or_role:
    ...
    async def command_disable_cog(self, ctx: commands.Context, *, cog: CogConverter):
        ...
        **Arguments:**
        ...
        if await self.bot._disabled_cog_cache.disable_cog_in_guild(cogname, ctx.guild.id):
            ...
            await ctx.send(
                _("{cogname} was already disabled (nothing to do).").format(
                    cogname=cogname)
            )
    ...
    async def command_enable_cog(self, ctx: commands.Context, *, cogname: str):
        ...
        **Arguments:**
        ...
        if await self.bot._disabled_cog_cache.enable_cog_in_guild(cogname, ctx.guild.id):
            ...
            await ctx.send(
                _("{cogname} was not disabled (nothing to do).").format(
                    cogname=cogname)
            )
    ...
    async def list_disabled_global(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        paged = [box(x) for x in pagify(
            humanize_list(disabled_list), page_length=1000)]
        ...
        **Example:**
        ...
        for channel_id in channel_ids:
            ...
            if channel:
                ...
                data.append(
                    _("Unknown channel with id: {id}").format(id=channel_id))
    ...
    async def count_ignored(self, ctx: commands.Context):
        ...
        cat_str = (
            humanize_list([c.name for c in category_channels]
                          ) if category_channels else "None"
        )
        chan_str = humanize_list(
            [c.mention for c in text_channels]) if text_channels else "None"
