...
class FixedVarious(commands.Cog):
    ...
    async def avatar(self, ctx: commands.Context, user: discord.User = None):
        ...
        if user is None:
            user = ctx.author
        avatar_url = user.display_avatar.replace(static_format="png")
    ...
    async def assets(self, ctx: commands.Context):
        ...
        dummy_asset = type("", (), {"replace": lambda format: None})
        img_dict = {"Server Logo": gld.icon.replace(static_format="png"),
                    "Server Banner": (gld.banner or dummy_asset).replace(format="png"),
                    "Server Invite Splash": (gld.splash or dummy_asset).replace(format="png")}
        embed_list = []
        for name, img_url in img_dict.items():
            if img_url:
                embed = discord.Embed(
                    colour=discord.Colour.blurple(), title=name)
    ...
    async def channel_slowmode(self, ctx: commands.Context, channel: discord.TextChannel, *,
        ...
        try:
            ...
            if seconds == 0:
                ...
                notice = self.SLOWMODE_SET.format(
                    mention, humanize_timedelta(timedelta=time))
    ...
    async def spotify(self, ctx: commands.Context, user: discord.Member = None):
        ...
        if user is None:
            user = ctx.author
        spot = next((act for act in user.activities if isinstance(
            act, discord.Spotify)), None)
        if spot:
            # Create value for song field.
            mins, secs = divmod(spot.duration.seconds, 60)
            song_length = "{:02d}:{:02d}".format(mins, secs)
            song_url = "https://open.spotify.com/track/{}".format(
                spot.track_id)
            song_value = "[{}]({}) ({})".format(
                spot.title, song_url, song_length)
            # Create embed.
            ...
            embed.add_field(name="Artist", value=",".join(
                spot.artists), inline=False)
    ...
    async def member_csv(self, ctx: commands.Context, delimiter: str = "\t"):
        ...
        if len(delimiter) != 1:
            ...
            csv_name = self.FOLDER + \
                "/{} at {}.csv".format(srv_name, file_stamp)
            with open(csv_name, 'w', newline='', errors="ignore", encoding='utf-8') as csv_f:
                ...
                for n, user in enumerate(sorted(gld.members, key=lambda x: x.joined_at)):
                    ...
                    userid = "ID: {}".format(
                        user.id)  # Excel truncates plain IDs :(
    ...
    async def role_population_embed(self, ctx: commands.Context, hierarchy_sort: bool = None):
        ...
        gld = ctx.guild
        role_tuples = ((r.mention, len(r.members), r.position)
                       for r in gld.roles if not self.ignore_role(r))
        if use_hierarchy:
            sorted_roles = sorted(
                role_tuples, key=lambda x: x[2], reverse=True)
            embed_footer = "Roles sorted on hierarchy."
        else:
            sorted_roles = sorted(
                role_tuples, key=lambda x: (x[1], x[2]), reverse=True)
