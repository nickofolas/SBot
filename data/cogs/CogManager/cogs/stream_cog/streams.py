...
from .streamtypes import PicartoStream, Stream, TwitchStream, YoutubeStream

MAX_RETRY_COUNT = 10

_ = lambda x: x  # fmt: off # noqa
...
class Streams(commands.Cog):
    ...
    def __init__(self, bot: Red):
        super().__init__()
        self.config: Config = Config.get_conf(self, 4355678345675678)
    ...
    async def _twitch(self, ctx: commands.Context, channel_name: str, user: Union[discord.Member, discord.User]):
        """Manage Twitch stream notifications."""
        await ctx.invoke(self.twitch_alert_channel, channel_name)

    @_twitch.command(name="channel")
    async def twitch_alert_channel(self, ctx: commands.Context, channel_name: str, user: Union[discord.Member, discord.User]):
        ...
        if re.fullmatch(r"<#\d+>", channel_name):
            ...
            return
        await self.stream_alert(ctx, TwitchStream, channel_name.lower(), user.id)

    @streamalert.command(name="youtube")
    async def youtube_alert(self, ctx: commands.Context, channel_name_or_id: str, user: Union[discord.Member, discord.User]):
        """Toggle alerts in this channel for a YouTube stream."""
        await self.stream_alert(ctx, YoutubeStream, channel_name_or_id, user.id)

    @streamalert.command(name="picarto")
    async def picarto_alert(self, ctx: commands.Context, channel_name: str, user: Union[discord.Member, discord.User]):
        """Toggle alerts in this channel for a Picarto stream."""
        await self.stream_alert(ctx, PicartoStream, channel_name, user.id)
    ...
    async def stream_alert(self, ctx: commands.Context, _class, channel_name, discord_user_id):
        ...
        if not stream:
            ...
            if is_yt and not self.check_name_or_id(channel_name):
                stream = _class(_bot=self.bot, id=channel_name, token=token,
                                config=self.config, user_id=discord_user_id)
            elif is_twitch:
                await self.maybe_renew_twitch_bearer_token()
                stream = _class(
                    _bot=self.bot,
                ...
                if is_yt:
                    stream = _class(
                        _bot=self.bot, name=channel_name, token=token, config=self.config, user_id=discord_user_id
                    )
                else:
                    stream = _class(_bot=self.bot, name=channel_name,
                                    token=token, user_id=discord_user_id)
            try:
    ...
    @streamset.command()
    @commands.guild_only()
    async def streamerrole(self, ctx: commands.Context, *, role: discord.Role):
        """Set a role to be assigned to streamers when they go live"""
        await self.config.guild(ctx.guild).streamer_role.set(role.id)
        await ctx.send(f"Streamer role set to {role.mention}", allowed_mentions=discord.AllowedMentions(roles=False))
            ...
            try:
                try:
                    ...
                    for channel_id in stream.channels:
                        ...
                        try:
                            if (role_id := await self.config.guild(channel.guild).streamer_role()):
                                member = await channel.guild.fetch_member(stream.discord_user_id)
                                await member.remove_roles(discord.Object(int(role_id)), reason="Auto-unassign streamer role")
                    ...
                    for channel_id in stream.channels:
                        ...
                        try:
                            if (role_id := await self.config.guild(channel.guild).streamer_role()):
                                member = await channel.guild.fetch_member(stream.discord_user_id)
                                await member.add_roles(discord.Object(int(role_id)), reason="Auto-assign streamer role")
